...
from models import Chunk, ChunkConfig, Document

logger = logging.getLogger(__name__)


class Chunker:
    """Splits documents into token-counted chunks with overlap."""

    def __init__(self, config: ChunkConfig):
        self.chunk_size = config.chunk_size
        self.overlap = config.overlap
        # Use cl100k_base tokenizer (GPT-4 / ada-002 compatible)
        self._enc = tiktoken.get_encoding("cl100k_base")

    def chunk_document(self, doc: Document) -> list[Chunk]:
        """Split a document into chunks.

        Each chunk inherits the document's metadata plus chunk-specific fields.
        """
