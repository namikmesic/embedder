...
class FAISSStore(VectorStore):
    """Vector store backed by FAISS with a JSON sidecar for metadata.

    Uses an IndexFlatIP (inner product) index, which is equivalent to
    ...
    async def delete(self, ids: list[str]) -> int:
        """Delete vectors by ID. Requires rebuilding the index."""
        id_set = set(ids)
    ...
    async def delete_by_metadata(self, filter_key: str, filter_value: str) -> int:
        """Delete all vectors matching a metadata key=value."""
        keep_indices = [
    ...
    def _rebuild_index(self, keep_indices: list[int]) -> None:
        """Rebuild the FAISS index keeping only specified indices."""
        if not keep_indices:
